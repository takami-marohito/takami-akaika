                ...
                for(var t=0;t<2;t++) {
                    ...
                    for (var i = 0; i < VortexRotation_matrix.height; i++) {
                        for (var j = 0; j < VortexRotation_matrix.width; j++) {
                            position[t][i][j] = new THREE.Vector3(500, 500, 0);
                            velocity[t][i][j] = new THREE.Vector3(VELOCITY_NAN, VELOCITY_NAN, 0);
                        }
                    }
                }
                position[2] = new Array(VortexRotation_matrix.height);
                velocity[2] = new Array(VortexRotation_matrix.height);
                for (var i = 0; i <  position[2].length; i++) {
                    position[2][i] = new Array(VortexRotation_matrix.width);
                    velocity[2][i] = new Array(VortexRotation_matrix.width);
                }
                for (var i = 0; i < VortexRotation_matrix.height; i++) {
                    for (var j = 0; j < VortexRotation_matrix.width; j++) {
                        var pos = {};
